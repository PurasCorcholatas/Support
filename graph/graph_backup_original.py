...
# Eres Santiago, t├®cnico de soporte de Serviunix. Hablas por WhatsApp como lo har├¡a cualquier persona real.
# Sin formalismos, sin sonar a bot, sin listas numeradas, sin signos de exclamaci├│n exagerados.

# C├ôMO HABLAS:
# - Corto y directo. M├íximo 2 oraciones.
# - Como alguien que conoce al usuario, no como un formulario
# - Usas expresiones naturales: "listo", "dale", "cu├®ntame","oye"
# - Nunca uses: "onda","che","mira" u otras expresiones regionales
# - Si el usuario saluda ÔåÆ respondes el saludo brevemente Y cumples el objetivo en la misma respuesta
# - Nunca repites informaci├│n que el usuario ya dio
# - Nunca haces m├ís de una pregunta
# - Nunca uses signos de exclamaci├│n

# EJEMPLOS DE TONO:
# Malo: "┬íHola! ┬┐En qu├® puedo ayudarte hoy?"
# Bueno: "Hola Simon como estas, ┬┐Te encuentras en Bello"

# Malo: "Por favor ind├¡came el nombre de tu empresa."
# Bueno: "┬┐Me podrias dar el nombre de tu empresa?"

# Malo: "Entendido. Proceder├® a verificar tu identidad."
# Bueno: "Ok, te voy a hacer unas preguntas r├ípidas para verificar que eres t├║."

# Malo: "┬íPerfecto! Tu registro ha sido completado exitosamente."
# Bueno: "Listo, quedaste registrado en esa sede. ┬┐En qu├® te puedo ayudar?"
# """),
...
#     response = llm.invoke([
#         SystemMessage(content="""
# Extrae ├║nicamente el nombre de la sede mencionada por el usuario.
# Devuelve SOLO el nombre de la sede, m├íximo 3 palabras.
...
#     sede = re.sub(r"[^a-z├í├®├¡├│├║├▒\s]", "", sede).strip()
...
#         "diagnosis_step": None,
#         "messages": [AIMessage(
#             content=f"Para {accion} necesito tu correo, {name}. ┬┐Cu├íl es?"
#         )]
#     }
...
#                 "greeting_step": "register_name",
#                 "messages": [
#                     AIMessage(content="Hola, Bienvenido al soporte t├®cnico de Serviunix. ┬┐Con qui├®n tengo el gusto?")
#                 ]
#             }
...
#             extract = llm.invoke([
#                 SystemMessage(content="Extrae SOLO el nombre propio de la persona. Devuelve ├║nicamente el nombre, sin m├ís texto."),
#                 HumanMessage(content=f"El usuario dijo: {last_user_message}")
#             ])
...
#                 "greeting_step": "register_company_sede",
#                 "messages": [AIMessage(
#                     content=f"Mucho gusto, {name}. ┬┐En qu├® empresa trabajas y desde qu├® sede te comunicas?"
#                 )]
#             }
...
#                 HumanMessage(content=f"""
# El usuario dijo esto cuando se le pregunt├│ empresa y sede: "{last_user_message}"
...
#                     SystemMessage(content="""
# Eres Santiago, soporte t├®cnico de Serviunix por WhatsApp.
# Responde brevemente a lo que dice el usuario y luego preg├║ntale
...
#             if sede:
#                 msg = f"Perfecto{', ' + name if name else ''}. Quedaste registrado desde {sede} en {empresa}. ┬┐En qu├® te puedo ayudar?"
#             else:
#                 msg = f"Perfecto{', ' + name if name else ''}. Registrado en {empresa}. ┬┐En qu├® te puedo ayudar?"
...
#                 last_user_message,
#                 {
#                     "confirma": "dice que s├¡, que sigue en esa sede",
#                     "niega": "dice que no est├í en esa sede",
#                     "otra_sede": "menciona una sede diferente",
#                     "otro": "habla de otra cosa"
#                 },
#                 context=f"El bot pregunt├│ si el usuario sigue en la sede {sede}"
#             )
...
#                 return {
#                     "greeting_step": "waiting_problem",
#                     "messages": [AIMessage(content=f"Perfecto. ┬┐En qu├® te puedo ayudar, {name}?")]
#                 }
...
#                     return {
#                         "greeting_step": "waiting_problem",
#                         "messages": [AIMessage(content=f"Listo, te actualizo a {sede_detected}. ┬┐En qu├® te puedo ayudar?")]
#                     }
#                 return {
#                     "greeting_step": "confirm_branch",
#                     "messages": [AIMessage(content=f"┬┐Desde qu├® sede te comunicas hoy, {name}?")]
#                 }
...
#             SystemMessage(content=(
#                 "Eres un t├®cnico de soporte. Describe en texto plano el error que ves en la imagen. "
#                 "Incluye: aplicaci├│n, c├│digo de error, mensaje exacto, sistema operativo o VM afectada, "
#                 "y cualquier otro detalle visible. M├íximo 4 oraciones."
#             )),
#             last_msg_obj
#         ])
...
# ┬┐La respuesta del usuario es una pregunta de aclaraci├│n sobre t├®rminos t├®cnicos o sobre c├│mo hacer algo que el t├®cnico mencion├│?
# (por ejemplo: "┬┐qu├® son los logs?", "┬┐c├│mo borro la cach├®?", "┬┐d├│nde veo eso?", "┬┐qu├® es eso?", "no entiendo", "┬┐qu├® es ping?", "┬┐c├│mo hago eso?")
...
#                 SystemMessage(content="""
# Eres Santiago, t├®cnico de soporte de Serviunix hablando por WhatsApp.
# El usuario no entendi├│ algo t├®cnico que le preguntaste.

# REGLAS ESTRICTAS:

# 1. PRIMERO detecta el nivel t├®cnico del usuario por c├│mo escribi├│ en el historial:
#    - T├ëCNICO: usa palabras como servidor, ping, SSH, IP, logs, firewall, DNS, terminal
#    - USUARIO FINAL: escribe casual, dice "no me abre", "no funciona", no usa t├®rminos t├®cnicos

# 2. Si pregunta por logs, terminal, SSH, comandos o configuraci├│n de servidor:
#    - Si es T├ëCNICO ÔåÆ expl├¡cale c├│mo hacerlo en 1 oraci├│n y repite la pregunta
#    - Si es USUARIO FINAL ÔåÆ dile "no te preocupes por eso, yo lo escalo" y pregunta algo observable

# 3. Si pregunta por algo del navegador (cach├®, cookies):
#    - Expl├¡calo en 1 oraci├│n simple y repite la pregunta

# 4. Si no entiende un t├®rmino t├®cnico:
#    - Trad├║celo a palabras cotidianas en 1 oraci├│n y repite la pregunta

# TRADUCCIONES A LENGUAJE SIMPLE:
# - "hacer ping" ÔåÆ "┬┐otros compa├▒eros tienen el mismo problema?"
# - "revisar logs" ÔåÆ "┬┐el error empez├│ de repente o despu├®s de alg├║n cambio?"
# - "cliente SMB" ÔåÆ "┬┐desde otro computador de la oficina puedes abrir esa carpeta?"
# - "servicio ca├¡do" ÔåÆ "┬┐otras cosas de la empresa tambi├®n fallan o solo esto?"
# - "resoluci├│n DNS" ÔåÆ "┬┐puedes abrir otras p├íginas o sistemas de la empresa?"
...
# La pregunta de diagn├│stico que hiciste fue:
# "{last_bot_message}"

# El usuario pregunt├│ o no entendi├│:
# "{last_user_message}"

# Responde seg├║n su nivel t├®cnico y luego repite la pregunta adaptada.
# """)
#             ])
#             return {
#                 "diagnosis_step": step,
...
# ┬┐Tienes suficiente para abrir un ticket ├║til?

# Considera que ES suficiente si tienes:
# - El error exacto o s├¡ntoma concreto (incluye im├ígenes descritas)
# - El sistema afectado con detalle
# - Cu├índo empez├│ o qu├® contexto hay (aunque sea "de repente sin cambios")
# - Si afecta a uno o varios usuarios

# NO es necesario saber qu├® intent├│ el usuario si ya est├í claro que no ha intentado nada 
...
#             "diagnosis_step": 1,
#             "diagnosis_history": [],
#             "messages": [AIMessage(content="Cu├®ntame qu├® est├í pasando y te ayudo a crear el ticket.")]
#         }

#     response = llm_diagnosis.invoke([
#     SystemMessage(content="""
# Eres Santiago, t├®cnico de soporte de Serviunix por WhatsApp.

# Tu ├║nica tarea: escribir UNA pregunta corta al usuario. Nada m├ís.

# FORMATO DE SALIDA OBLIGATORIO:
# - Solo la pregunta. Sin introducci├│n, sin an├ílisis, sin separadores.
# - M├íximo 1 oraci├│n.
# - Sin signos de exclamaci├│n.

# EJEMPLOS DE LO QUE DEBES DEVOLVER:
# "┬┐Otras VMs tambi├®n fallan o solo esa?"
# "┬┐Esto empez├│ despu├®s de alg├║n cambio o de repente?"
# "┬┐Solo a ti te pasa o tambi├®n a compa├▒eros?"

# EJEMPLOS DE LO QUE JAM├üS DEBES DEVOLVER:
# "Por el historial, este usuario es t├®cnico..." ÔåÉ PROHIBIDO
# "---" ÔåÉ PROHIBIDO  
# "Seg├║n lo que describes..." ÔåÉ PROHIBIDO
# Cualquier texto antes o despu├®s de la pregunta ÔåÉ PROHIBIDO

# Si ya tienes el error de una imagen, no preguntes qu├® error tiene.
# Pregunta por lo que a├║n no sabes: cu├índo empez├│, si afecta a otros, qu├® ya intentaron.
# """),
#     HumanMessage(content=f"""
# HISTORIAL:
# {updated_history_text}

# Devuelve SOLO la siguiente pregunta. Una oraci├│n. Sin nada m├ís.
# """)
# ])
...
#             last,
#             {
#                 "confirma": "dice que s├¡, es el mismo problema, no es algo nuevo",
...
#                 contexto="No qued├│ claro si es el mismo problema.",
#                 historial=[last]
#             )]
#         }
...
#             contexto="El usuario no eligi├│ una opci├│n v├ílida.",
#             historial=[last]
#         )]
#     }
...
#         if not customer:
#             print(f"[support_agent] WARN: usuario {thread_id} sin email v├ílido, ticket se crear├í con thread_id como customer")
...
#         )),
#         HumanMessage(content=f"""
# Convierte este diagn├│stico en un ticket de soporte.

# REGLAS:
# - La descripci├│n debe ser un p├írrafo continuo, natural, como un resumen t├®cnico.
# - Incluye TODOS los datos concretos que aparezcan en el historial: horas, fechas, errores exactos, sistemas afectados, pasos ya intentados, cu├íntos usuarios afectados.
# - Si el usuario mencion├│ una hora o fecha, escr├¡bela textualmente en la descripci├│n.
# - No uses listas ni bullets. Solo p├írrafo.
# - La descripci├│n debe tener m├¡nimo 3 oraciones.
...
# TITULO: [titulo corto m├íximo 10 palabras]
# DESCRIPCION: [p├írrafo detallado con todos los datos del historial]
# """)
#     ])
...
#                 "messages": [AIMessage(content="No veo el n├║mero del ticket claro, ┬┐me lo puedes repetir? Solo el n├║mero, sin letras ni s├¡mbolos.")]
#             }
...
#                 "messages": [AIMessage(content="No pude consultar el ticket ahorita, intenta m├ís tarde.")]
#             }

#         result = await search_tool.ainvoke({"params": {"query": ticket_number}})

#         ticket_info = "No encontr├® informaci├│n sobre ese ticket."
...
#             "messages": [AIMessage(
#                 content=f"Mira, esto encontr├®:\n\n{ticket_info}"
#                 if "Ticket:" in ticket_info
#                 else f"No encontr├® un ticket con el n├║mero {ticket_number}, ┬┐me confirmas el n├║mero correcto?"
#             )]
#         }
...
#         "ticket_status_step": "ask_id",
#         "messages": [guided_response(
#             objetivo="Pedirle al usuario el n├║mero del ticket que quiere consultar.",
...
#     1. Si el agente humano ya tom├│ el caso
...
#         return {
#             "email_request_step": "ask_email",
#             "messages": [AIMessage(content="Ese correo no parece v├ílido, ┬┐me lo confirmas?")]
#         }
...
#                 "password": "tiene problema con contrase├▒a o acceso",
...
#             },
#             context="El usuario estaba en medio de un flujo de soporte (diagnostico, opciones, o contrase├▒a)."
#         )
...
#     5. Diagnosis activo ÔÇö ANTES del bloque waiting_problem
#     if state.get("diagnosis_step") is not None:
#         return {"intent": "diagnosis_flow"}
...
#     9. greeting_step == waiting_problem ÔåÆ detectar intent del mensaje nuevo
...
#                 "password": "tiene problema con contrase├▒a, clave o acceso al correo",
...
#     10. Primer mensaje ÔÇö usuario conocido o nuevo
...
# - Nunca digas "describe tu problema" o "proporcione informaci├│n"
# - Usa frases como: "cu├®ntame qu├® est├í pasando", "revisemos eso", "mira, puede ser que..."
# - Sin signos de exclamaci├│n, sin listas innecesarias

# Haz preguntas de diagn├│stico cuando haya un problema t├®cnico.
# """
...
#         cuerpo = f"""
# ESCALACI├ôN A SOPORTE HUMANO

# INFORMACI├ôN DEL CLIENTE
# Nombre: {nombre}
# Empresa: {empresa}
# Correo: {correo}
# Tel├®fono / ID Conversaci├│n: {thread_id}

# CONTEXTO COMPLETO DE LA CONVERSACI├ôN
# {historial}

# ACCESO DIRECTO A CHATWOOT
# {chatwoot_link}

# Este caso requiere intervenci├│n manual.
# """

#         msg = MIMEText(cuerpo)
#         msg["Subject"] = f"Escalaci├│n Soporte - {nombre} ({empresa})"
#         msg["From"] = smtp_email
#         msg["To"] = destino
...
#         return {
#             "messages": [
#                 AIMessage(content="Listo, ya avis├® al equipo. En breve te escribe un t├®cnico.")
#             ]
#         }
...
#         return {
#             "messages": [AIMessage(content="Con gusto, ya est├ín al tanto.")]
#         }

#     return {
#         "messages": [AIMessage(content="Ya avis├®, en breve te contactan. Si necesitas cancelar la espera dime.")]
#     }


# def normalize(text: str):
#     text = text.lower().strip()
#     text = re.sub(r'[^a-z0-9├í├®├¡├│├║├▒]', '', text)
#     return text
