...
class ZammadService:
    ...
    def create_user_if_not_exists(email: str, first_name: str = "Cliente", last_name: str = ""):
        ...
        if users_found:
            ...
            url_update = f"{ZAMMAD_URL}/users/{user_id}"
            payload = {
            ...
            requests.put(url_update, json=payload, headers=headers)
            return user_id
    ...
    @staticmethod
    def add_note(ticket_id: int, body: str, internal: bool = True):
        ...
        payload = {
            "ticket_id": ticket_id,
            "subject": "Validación automática del bot",
            "body": body,
            "type": "note",
            "internal": internal
        }
        ...
        if response.status_code != 201:
            print("Error agregando nota:", response.text)
